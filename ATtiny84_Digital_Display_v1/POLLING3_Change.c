 ...
 *
 *
 *	This time tried to add a ~= 20 second delay
 *	when decrementing the temperature,
 *	the result was at first well, but the delay was  a bit too much
 *
 *	Settled on going back to POLLING3_Change, but with a twist
 *	an alarm to warn the user that "YOU ARE ABOUT TO RUN OUT OF HOT WATER"
 *  "EL Sa5an Bykalemak"
 *
...
void APP_raiseAdcFlag(void)
{
 ...
 /*Multiplex the two digit with a refresh rate = 8 milliseconds */
...
uint8 getTemp()
{
/* OLD CODE (failed attempt):
 * Tried to calculate the average temp of a no. of "SIZE" readings
 *
	unsigned char i = 0;
...
int main()
{
	...
	while(1)
	{
		...
		if(g_adc)
		{
			...
			/*check if old and new temperature readings do not match
			 * Increasing State ****/
			if(ntcTemp > ntcTemp_old)
			{
				...
				if(tempChangeCount < 10)
					...
					i.e; the temp is still fluctuating*/
					ntcTemp = ntcTemp_old;
				else
				{
					ntcTemp_old = ntcTemp;
			...
			else if(ntcTemp < ntcTemp_old)
			{
				 ...
				 * assign new temp to old temp*/
				if(tempChangeCount < 10)
					/*if count is less than a certain threshold, keep the old temp,
					i.e; the temp is still fluctuating*/
					ntcTemp = ntcTemp_old;
				else
				{
					...
					if((ntcTemp_old <= 45) && (ntcTemp_old >= 40))
					{
						Temp45_flag = 1;
					}
				}
