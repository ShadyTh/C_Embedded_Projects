...
uint8 tempDecreaseCount = 0;

uint8 g_tick_t0 = 0;
uint8 g_tick_t1 = 0;

uint8 temp_readings[100] = {1};
uint8 index = 0;
...
void APP_raiseAdcFlag(void)
{
	...
	g_tick_t0 ++;
	if(g_tick_t0 % 2 == 0)		/*after 0.5 second (2*250ms) */
		{
			g_adc = 1;
			g_tick_t0 = 0;
		}
...
int main()
{
	...
	while(1)
	{
		...
		if(g_adc)
		{
			...
			/* check if temp is decreasing; i.e: Cold water is starting to flow in */
			if(ntcTemp < ntcTemp_old)
			{
				tempDecreaseCount++;

				if(tempDecreaseCount < 50)
					ntcTemp = ntcTemp_old;
				else
				{
					ntcTemp_old --;
